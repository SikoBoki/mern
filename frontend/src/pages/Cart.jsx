...
const stripePromise = loadStripe(
	'pk_test_51PtoEJP5ieA9DFSiBVFBjMSTnaqKfIbbttDWzl5cvi93dSncrUgf1PQNI22FjypC85Jm85ZZg1e1gbXNZ5lglsDO00oVLqsWYO'
)
	...
	const handleCheckout = async () => {
		...
		try {
			const response = await axios.post(
				`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/stripe/create-checkout-session`,
				{
					...
					<div key={index} className='bg-base-200 rounded-lg shadow-lg p-4 flex flex-col'>
						...
						<div className='flex items-center justify-between text-md mb-3'>
							...
							<div className='flex items-center'>
								<button onClick={() => decreaseCartItemQuantity(item._id)} className='  font-bold hover:underline'>
									Remove
								</button>
							</div>
						</div>
